...
type PointsAggregateRow = {
  totalPoints: number | bigint | null;
};
...
async function getCustomerProfileData(userId: number) {
  ...
  const [
    ...
    await Promise.all([
      ...
      customer.referredBy
        ...
        : null,
      prisma.$queryRaw<PointsAggregateRow[]>`
...
export async function getCustomerCoupons(
  ...
  try {
    ...
    const promotions = await prisma.$queryRaw<CustomerCouponRow[]>`
      SELECT
        up.id,
        up.status,
        up."createdAt",
      ...
      data: {
        ...
        coupons: promotions.map((promotion) => ({
          ...
          usedAt: promotion.usedAt
            ...
            : null,
          status:
            promotion.status === "USED" || promotion.status === "EXPIRED"
              ? promotion.status
              : "ACTIVE",
        })),
      },
